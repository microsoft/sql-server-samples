...
**Languages are different: they are determined, not merely planned.** [§4 Article 3](#4-article-3-language-specific-requirements) fully specifies .NET, Python, and Java as target contracts — runtime/package versions, authentication pattern, configuration, vector parameter binding, bulk-insert pattern, the DiskANN row-count gate, package-manager convention, sample folder naming, and CI shape are all decided below. A contributor can start implementation directly from those sections without first resolving open architecture questions; only real execution evidence, and a small number of explicitly flagged version-pin confirmations, remain outstanding per language. Go remains explicitly out of scope for this scenario per the source plan (see [ASV-LANG-GO-1](#45-go-not-yet-implemented-explicitly-out-of-scope-for-v1)).
...
- **ASV-LANG-GO-1 (MUST, source: plan PDF p.3)** Go is explicitly out of scope for the first vector search scenario. Unlike .NET, Python, and Java, no target contract is specified for Go here because the source plan provides no driver, auth, or code evidence for it — do not add a Go sample under this scenario's v1 scope without first authoring a full `4.N` Go subsection (per [ASV-LANG-NEW-2](#46-adding-a-new-language-process-requirement)) and superseding this requirement.

### 4.6 Adding a new language — process requirement

- **ASV-LANG-NEW-1 (MUST)** .NET ([§4.2](#42-net-not-yet-implemented-fully-specified-target-contract)), Python ([§4.3](#43-python-not-yet-implemented-fully-specified-target-contract)), and Java ([§4.4](#44-javajdbc-not-yet-implemented-fully-specified-target-contract-reference-priority)) already have fully specified target contracts above — implement directly against those sections; do not re-derive requirements from the plan PDF or invent new ones for those three languages.
- **ASV-LANG-NEW-2 (MUST)** For Go (currently out of scope, [ASV-LANG-GO-1](#45-go-not-yet-implemented-explicitly-out-of-scope-for-v1)) or any language beyond the five named in the source plan: (a) read Articles 1 and 2 in full, (b) confirm the dataset/embedding-model/canonical-query parity requirements in [ASV-CORE-18](#2-article-1-program-wide-requirements-all-azure-sql-vector-work)/[ASV-CORE-19](#2-article-1-program-wide-requirements-all-azure-sql-vector-work), (c) add a new `4.N` subsection following the same structure as [§4.2](#42-net-not-yet-implemented-fully-specified-target-contract)–[§4.4](#44-javajdbc-not-yet-implemented-fully-specified-target-contract-reference-priority) (runtime/package, auth, config, vector binding, bulk insert, DiskANN gate, package manager, folder naming, CI), and (d) update [§1 Reality snapshot](#1-reality-snapshot-read-this-first) and `samples/features/vector-search/README.md`'s language table in the same PR once a sample is merged, changing its status from "fully specified target contract" to `Evidence: implemented`.
