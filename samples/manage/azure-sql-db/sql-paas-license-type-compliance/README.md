...
3. Select the definition location, and enter a name such as `Configure Azure SQL Database license type` and a category.
...
6. On the **Parameters** tab, leave **Effect** set to `DeployIfNotExists` and select the **Target license type**: `LicenseIncluded` for Pay-as-you-go or `BasePrice` for Azure Hybrid Benefit.
7. On the **Remediation** tab, select **Create a Managed Identity**, choose an identity location, and select **Create a remediation task** if existing non-compliant databases should be updated.
...
Selecting `BasePrice` confirms that you meet the Azure Hybrid Benefit licensing conditions described above.
