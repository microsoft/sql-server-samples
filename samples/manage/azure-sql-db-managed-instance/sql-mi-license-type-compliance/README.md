...
3. Select the definition location, and enter a name such as `Configure SQL Managed Instance license type` and a category.
...
7. On the **Remediation** tab, select **Create a Managed Identity**, choose an identity location, and select **Create a remediation task** if existing non-compliant managed instances should be updated.
...
Selecting `BasePrice` or `HybridFailoverRights` confirms that you meet the corresponding licensing conditions described above.
